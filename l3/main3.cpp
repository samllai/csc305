	...
	for (int i = 0; i < world.width; i++) {
		for (int j = 0; j < world.height; j++) {
			...
			for (int s = 0; s < world.sampler->getNumSamples; s++) {
				...
				pixel_point.x = (i - 0.5f * (world.width - 1.0f));
				pixel_point.y = (j - 0.5f * (world.height - 1.0f));
				pixel_point.z = dist;
				sample_point += pixel_point;
				ray.d = glm::normalize(sample_point);

				for (auto&& object : world.scene) {
					object->hit(ray, sr);
				}
				if (sr.t = std::numeric_limits<float>::max()) {
					avg_colour = world.background;
				}
				else {
					avg_colour /= world.sampler->getNumSamples;
				}
...
int main()
{
	...
	Pinhole phcam {1000.0f};
	World w;
	w.width = 600;
	w.height = 600;
	w.background = { 0,0,0 };

	std::shared_ptr<Regular> sample = std::make_shared<Regular> ( 16, 16 );
